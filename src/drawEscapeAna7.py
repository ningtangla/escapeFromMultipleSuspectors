...
def main():
    ...
    manipulatedVariables['aliveBouns'] = [0.0, 0.05, 0.1, 0.2]
    ...
    toDropLevels = ['alpha', 'C', 'cBase', 'numTrees', 'rangeAtt', 'minAttDist', 'damp', 'softSubtlety',
    ...
    numRows = len(manipulatedVariables['aliveBouns'])
    plotCounter = 1
    for key, group in modelResultDf.groupby(['aliveBouns', 'actCost']):
        ...
        group = group.stack()
        group.index.names = ['softId', 'actCost', 'aliveBouns', 'chasingSubtlety']
        group.index = group.index.droplevel(['actCost', 'aliveBouns'])
