...
class RunMCTSTrjactory:
    ...
    def __call__(self, mcts):
        ...
        for runningStep in range(self.maxRunningSteps):
            ...
            stateToRecord = [physicalState, posterior, attentionStatus, [np.array(list(rootNode.id.values())[0])[0][3] for rootNode in rootNodes]] 
...
class RunOneCondition:
    ...
    def __call__(self, condition):
        ...
        possibleTrialSubtleties = [11.0, 3.3]#[500.0, 11.0, 3.3, 1.83, 0.92, 0.31, 0.001]
        for subIndex in range(numSub):
            ...
            for chasingSubtlety in possibleTrialSubtleties: 
               ...
               # render = env.Render(numAgent, screen, xBoundary[1], yBoundary[1], screenColor, sheepColor, wolfColor, circleSize, saveImage, saveImageFile, isTerminal)
                ...
                savePathIdentitySampledSeries = getCSVSavePath({'chasingSubtlety': chasingSubtlety, 'measure': 'identitySampled'})
                if subIndex == 0:
                    resultsSampledTimeSeries.to_csv(savePathIdentitySampledSeries, mode='a')
                else:
                    resultsSampledTimeSeries.to_csv(savePathIdentitySampledSeries, mode='a', header=False)
                meanIdentitySampled = np.mean([getSampledWolfIndentityAcc(trajectory) for trajectory in trajectories])
                meanIdentitySampledOnConditions.update({chasingSubtlety: meanIdentitySampled})
            ...
            csvSavePathIdentitySampled = getCSVSavePath({'measure': 'identitySampled'})
...
def main():     
    ...
    manipulatedVariables['rangeAttention'] = [10.0]#[5.0, 10.0, 20.0, 40.0]
