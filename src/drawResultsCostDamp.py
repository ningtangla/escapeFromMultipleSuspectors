...
def main():     
    ...
    manipulatedVariables['actionCost'] = [0.0, 0.01, 0.05]
    manipulatedVariables['damp'] = [0.0, 0.05, 0.15, 0.25]
    manipulatedVariables['cBase'] = [40]
    manipulatedVariables['numTrees'] = [2]
    manipulatedVariables['numSimulationTimes'] = [90]
    ...
    fig = plt.figure()
    numColumns = len(manipulatedVariables['actionCost'])
    numRows = len(manipulatedVariables['damp'])
    plotCounter = 1
    for key, group in modelResultDf.groupby(['damp', 'actionCost']):
        ...
        group = group.stack()
        group.index.names = ['attentionType', 'actionCost', 'damp', 'chasingSubtlety']
        group.index = group.index.droplevel(['actionCost', 'damp'])
        ...
        for attentionType, grp in group.groupby('attentionType'):
            ...
            if str(attentionType) == manipulatedVariables['attentionType'][-1]:
                ...
                grp['human'] = [0.6, 0.37, 0.25, 0.24, 0.51]
