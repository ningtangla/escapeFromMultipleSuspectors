...
def main():     
    ...
    manipulatedVariables['minAttentionDistance'] = [10.0, 40.0]
    manipulatedVariables['rangeAttention'] = [10.0]#, 6.2, 6.3]
    manipulatedVariables['cBase'] = [50]
    manipulatedVariables['numTrees'] = [2]
    manipulatedVariables['numSimulationTimes'] = [103]
    manipulatedVariables['actionRatio'] = [0.05]
    ...
    toDropLevels = ['alphaForStateWidening', 'CForStateWidening', 'cBase', 'numTrees', 'rangeAttention', 'actionRatio', 'burnTime',
            'measure', 'attentionType', 'numSimulationTimes']
    modelResultDf.index = modelResultDf.index.droplevel(toDropLevels)
    fig = plt.figure()
    numColumns = len(manipulatedVariables['softId'])
    numRows = len(manipulatedVariables['softSubtlety'])
    plotCounter = 1
    for key, group in modelResultDf.groupby(['softSubtlety', 'softId']):
        ...
        group = group.stack()
        group.index.names = ['minAttentionDistance', 'softId', 'softSubtlety', 'chasingSubtlety']
        group.index = group.index.droplevel(['softId', 'softSubtlety'])
        ...
        for attentionType, grp in group.groupby('minAttentionDistance'):
            grp.index = grp.index.droplevel('minAttentionDistance')
            if str(attentionType) == manipulatedVariables['minAttentionDistance'][-1]:
                grp['human'] = [0.24, 0.51]
